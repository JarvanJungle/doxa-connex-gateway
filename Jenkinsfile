pipeline {
  ...
  environment {
    ...
    DEV_REPO = 'doxa-connex-dev/gateway'
    UAT_REPO = 'doxa-connex-uat/gateway'
    STAG_USERID = '538871320653'
    UAT_USERID = '556257862131'
    STAG_REPO = 'doxa-connex-stag/gateway'
    PROD_REPO = 'doxa-connex-prod/gateway'
    ...
    stage('Set variables') {
      parallel {
        stage('Set variables for STAGE') {
          when { anyOf { branch 'release/develop';branch 'release/stag'; branch 'release/uat'; branch 'release/production';} }
          steps {
            script {
    ...
    stage('Deploy to kubernetes') {
      ...
      steps {
        ...
        withKubeConfig([credentialsId: "${DEF_KUBE_CREDENTIALS}", serverUrl: "${CLUSTER}"]) {
          ...
          // sh "/usr/local/bin/kubectl apply -f ./deployment/gateway.ingress-${STAGE}.yml --namespace ${DEF_NAMESPACE}"
        }
      }
    }
